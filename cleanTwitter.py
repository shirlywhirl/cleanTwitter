...
class TwitterClean:
    ...
    def authenticate_from_config(self, config_path=None):
        ...
        try:
            ...
            try:
                ...
                print(
                    "Please check the [Authentication] information in your configuration file."
                )
                self.api = None
            else:
                if all([ck, cs, at, ats]):
                    ...
                    self.api = None
                    print(
                        "Please check the options set under [Authentication] in your configuration file."
                    )
    ...
    def block_followers(self, target_user):
        for follower_id in self.limit_handled(
            tweepy.Cursor(self.api.followers_ids, id=target_user).items()
        ):
            try:
            ...
            for favorite in page:
                try:
                    if favorite.created_at < datetime.now() - timedelta(days=max_age):
                        print(
                            ">> %s unliking old tweet from %s"
                            % (favorite.id, favorite.created_at)
                        )
                        self.unlike(favorite.id)
                    else:
                        print(
                            ">> %s keeping like on old tweet from %s"
                            % (favorite.id, favorite.created_at)
                        )
                except:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Unlike or delete (re-)tweets (and optionally export them first). Set other parameters via configuration file (default: "settings.ini" in script directory) or arguments. Set arguments will overrule the configuration file.'
    )
    parser.add_argument(
        "--block",
        default=None,
        dest="target_user",
        help="Target user to block",
        type=str,
        action="store",
    )
    parser.add_argument(
        "--blockchain",
        default=None,
        dest="target_user_chain",
        help="Target user to block followers",
        type=str,
        action="store",
    )
    parser.add_argument(
        ...
        default=None,
        dest="untweet_id",
        help="Target id to delete if not retweet",
        type=str,
        action="store",
    )
    parser.add_argument(
